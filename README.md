...
for themselves would have scored **15.5** on Team Goal Coverage, a computed baseline rather than observed behaviour. The coordinated plan scores
...
sequenceDiagram
    ...
    H->>P: The same write queues parsing through the action retrier
...
| Demo runs, workflow imports, briefs and judge emails are rate-limited | Token buckets per conference for the brief, per browser visitor id for demo runs and workflow imports, and per demo code for judge emails, checked before any work; quota protection, not authentication, with no global cap | `convex/model/rateLimits.ts`                                   |
...
| Demo workspaces expire                                                | Removed by a six-hourly cleanup once more than 24 hours old; a workspace's link is its access key (see Known limitations)                                                                                                    | `convex/guest.ts`                                              |
